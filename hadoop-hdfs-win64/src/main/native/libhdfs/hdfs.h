...
extern  "C" {
...
#if defined(_WIN64)
#if defined(HDFS_EXPORTS)
	#define HDFS_API __declspec(dllexport)
#else
	#define HDFS_API __declspec(dllimport)
#endif
#else
	#define HDFS_API
#endif
     ...
     */
    HDFS_API int hdfsFileIsOpenForRead(hdfsFile file);
     ...
     */
    HDFS_API int hdfsFileIsOpenForWrite(hdfsFile file);
     ...
     */
    HDFS_API  int hdfsFileGetReadStatistics(hdfsFile file,
     ...
     */
    HDFS_API  int64_t hdfsReadStatisticsGetRemoteBytesRead(
     ...
     */
    HDFS_API void hdfsFileFreeReadStatistics(struct hdfsReadStatistics *stats);
     ...
     */
     HDFS_API hdfsFS hdfsConnectAsUser(const char* nn, tPort port, const char *user);
     ...
     * @deprecated Use hdfsBuilderConnect instead. 
     */
     HDFS_API  hdfsFS hdfsConnect(const char* nn, tPort port);
     ...
     * @deprecated   Use hdfsBuilderConnect instead. 
     */
     HDFS_API hdfsFS hdfsConnectAsUserNewInstance(const char* nn, tPort port, const char *user );
     ...
     * @deprecated   Use hdfsBuilderConnect instead. 
     */
     HDFS_API hdfsFS hdfsConnectNewInstance(const char* nn, tPort port);
     ...
     * @return       Returns a handle to the filesystem, or NULL on error.
     */
     HDFS_API hdfsFS hdfsBuilderConnect(struct hdfsBuilder *bld);
     ...
     */
    HDFS_API struct hdfsBuilder *hdfsNewBuilder(void);
     ...
     * @param bld The HDFS builder
     */
    HDFS_API void hdfsBuilderSetForceNewInstance(struct hdfsBuilder *bld);
     ...
     */
    HDFS_API void hdfsBuilderSetNameNode(struct hdfsBuilder *bld, const char *nn);
     ...
     * @param port The port.
     */
    HDFS_API void hdfsBuilderSetNameNodePort(struct hdfsBuilder *bld, tPort port);
     ...
     */
    HDFS_API void hdfsBuilderSetUserName(struct hdfsBuilder *bld, const char *userName);
     ...
     */
    HDFS_API void hdfsBuilderSetKerbTicketCachePath(struct hdfsBuilder *bld,
     ...
     * @param bld The HDFS builder
     */
    HDFS_API void hdfsFreeBuilder(struct hdfsBuilder *bld);
     ...
     */
    HDFS_API int hdfsBuilderConfSetStr(struct hdfsBuilder *bld, const char *key,
                              const char *val);
     ...
     */
    HDFS_API int hdfsConfGetStr(const char *key, char **val);
     ...
     *                 Failure to find the key is not an error.
     */
    HDFS_API int hdfsConfGetInt(const char *key, int32_t *val);
     ...
     */
    HDFS_API void hdfsConfStrFree(char *val);
     ...
     */
    HDFS_API int hdfsDisconnect(hdfsFS fs);
     ...
     */
    HDFS_API hdfsFile hdfsOpenFile(hdfsFS fs, const char* path, int flags,
     ...
     *         error.
     */
    HDFS_API int hdfsCloseFile(hdfsFS fs, hdfsFile file);
     ...
     * @return Returns 0 on success, -1 on error.  
     */
    HDFS_API int hdfsExists(hdfsFS fs, const char *path);
     ...
     * @return Returns 0 on success, -1 on error.  
     */
    HDFS_API int hdfsSeek(hdfsFS fs, hdfsFile file, tOffset desiredPos); 
     ...
     */
    HDFS_API tOffset hdfsTell(hdfsFS fs, hdfsFile file);
     ...
     */
    HDFS_API tSize hdfsRead(hdfsFS fs, hdfsFile file, void* buffer, tSize length);
     ...
     * @return      See hdfsRead
     */
    HDFS_API tSize hdfsPread(hdfsFS fs, hdfsFile file, tOffset position,
     ...
     */
    HDFS_API tSize hdfsWrite(hdfsFS fs, hdfsFile file, const void* buffer,
                    tSize length);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsFlush(hdfsFS fs, hdfsFile file);
     ...
     */
    HDFS_API int hdfsHFlush(hdfsFS fs, hdfsFile file);
     ...
     * @return 0 on success, -1 on error and sets errno
     */
    HDFS_API int hdfsHSync(hdfsFS fs, hdfsFile file);
     ...
     */
    HDFS_API int hdfsAvailable(hdfsFS fs, hdfsFile file);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsCopy(hdfsFS srcFS, const char* src, hdfsFS dstFS, const char* dst);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsMove(hdfsFS srcFS, const char* src, hdfsFS dstFS, const char* dst);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsDelete(hdfsFS fs, const char* path, int recursive);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsRename(hdfsFS fs, const char* oldPath, const char* newPath);
     ...
     */
    HDFS_API char* hdfsGetWorkingDirectory(hdfsFS fs, char *buffer, size_t bufferSize);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsSetWorkingDirectory(hdfsFS fs, const char* path);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsCreateDirectory(hdfsFS fs, const char* path);
     ...
     * @return Returns 0 on success, -1 on error. 
     */
    HDFS_API int hdfsSetReplication(hdfsFS fs, const char* path, int16_t replication);
     ...
     */
    HDFS_API hdfsFileInfo *hdfsListDirectory(hdfsFS fs, const char* path,
                                    int *numEntries);
     ...
     * NULL on error.
     */
    HDFS_API hdfsFileInfo *hdfsGetPathInfo(hdfsFS fs, const char* path);
     ...
     */
    HDFS_API void hdfsFreeFileInfo(hdfsFileInfo *hdfsFileInfo, int numEntries);
     ...
     */
    HDFS_API int hdfsFileIsEncrypted(hdfsFileInfo *hdfsFileInfo);
     ...
     * NULL on error.
     */
    HDFS_API char*** hdfsGetHosts(hdfsFS fs, const char* path, 
     ...
     * @param numEntries The size of the array.
     */
    HDFS_API void hdfsFreeHosts(char ***blockHosts);
     ...
     */
    HDFS_API tOffset hdfsGetDefaultBlockSize(hdfsFS fs);
     ...
     * @return              Returns the default blocksize, or -1 on error.
     */
    HDFS_API tOffset hdfsGetDefaultBlockSizeAtPath(hdfsFS fs, const char *path);
     ...
     */
    HDFS_API tOffset hdfsGetCapacity(hdfsFS fs);
     ...
     */
    HDFS_API tOffset hdfsGetUsed(hdfsFS fs);
     ...
     */
    HDFS_API int hdfsChown(hdfsFS fs, const char* path, const char *owner,
                  const char *group);
     ...
     * @return 0 on success else -1
     */
    HDFS_API   int hdfsChmod(hdfsFS fs, const char* path, short mode);
     ...
     * @return 0 on success else -1
     */
    HDFS_API int hdfsUtime(hdfsFS fs, const char* path, tTime mtime, tTime atime);
     ...
     */
    HDFS_API struct hadoopRzOptions *hadoopRzOptionsAlloc(void);
     ...
     */
    HDFS_API int hadoopRzOptionsSetSkipChecksum(
     ...
     */
    HDFS_API int hadoopRzOptionsSetByteBufferPool(
     ...
     */
    HDFS_API void hadoopRzOptionsFree(struct hadoopRzOptions *opts);
     ...
     *                   supplied.
     */
    HDFS_API struct hadoopRzBuffer* hadoopReadZero(hdfsFile file,
     ...
     */
    HDFS_API int32_t hadoopRzBufferLength(const struct hadoopRzBuffer *buffer);
     ...
     */
    HDFS_API const void *hadoopRzBufferGet(const struct hadoopRzBuffer *buffer);
     ...
     */
    HDFS_API void hadoopRzBufferFree(hdfsFile file, struct hadoopRzBuffer *buffer);
